...
import { convertToJson, readTranslationsFromWorksheet } from '../src/main.js';
...
describe('Path safety and duplicate detection', () => {
    ...
    it('rejects invalid language headers and prevents path traversal', async () => {
        ...
        try {
            ...
            threw = true;
            assert.match(String(e.message), /Invalid language code|Unsafe output path/);
        }
        assert.equal(threw, true, 'convertToJson should reject invalid language header');

        // Ensure no files were written
        const files = await fs.readdir(outDir);
        assert.equal(files.length, 0, 'no files should be written on invalid input');
    });
    ...
    it('fails on duplicates when failOnDuplicates is true (API)', async () => {
        ...
        assert.equal(threw, true, 'convertToJson should throw on duplicates when failOnDuplicates is true');
    });
    ...
    it('CLI: --fail-on-duplicates exits non-zero', async () => {
        ...
        const proc = spawn(process.execPath, [path.resolve(__dirname, '..', 'cli.js'), 'excel-to-i18n', '-i', xlsxPath, '-o', outDir, '--fail-on-duplicates'], {
        ...
        assert.notEqual(code, 0, 'CLI should exit non-zero on duplicates with --fail-on-duplicates');
